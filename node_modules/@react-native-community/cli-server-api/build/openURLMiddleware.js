...
function _strictUrlSanitise() {
  const data = require("strict-url-sanitise");
  _strictUrlSanitise = function () {
    return data;
  };
  return data;
}
...
async function openURLMiddleware(req, res, next) {
  if (req.method === 'POST') {
    ...
    let sanitizedUrl;
    try {
      sanitizedUrl = (0, _strictUrlSanitise().sanitizeUrl)(url);
    } catch {
      res.writeHead(400);
      res.end('Invalid URL');
      return;
    }
    await (0, _open().default)(sanitizedUrl);
    res.writeHead(200);
    res.end();
  }
  next();
