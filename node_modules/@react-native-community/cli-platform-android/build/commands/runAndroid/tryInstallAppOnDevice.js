...
function tryInstallAppOnDevice(args, adbPath, device, androidProject, selectedTask) {
  try {
    var _ref, _variantFromSelectedT;
    ...
    const variantPath = variantFromSelectedTask ? `${variantFromSelectedTask.slice(0, -1).join('')}/${variantFromSelectedTask.at(-1).toLocaleLowerCase()}` : defaultVariant;
    // create output file name, eg. `production-debug`
    const variantAppName = (variantFromSelectedTask === null || variantFromSelectedTask === void 0 ? void 0 : (_variantFromSelectedT = variantFromSelectedTask.join('-')) === null || _variantFromSelectedT === void 0 ? void 0 : _variantFromSelectedT.toLowerCase()) ?? defaultVariant;
    let pathToApk;
    if (!args.binaryPath) {
