...
async function runOnAllDevices(args, cmd, adbPath, androidProject) {
  ...
  if (devices.length === 0) {
    ...
    if (result.success) {
      ...
      _cliTools().logger.error(`Failed to launch emulator. Reason: ${_picocolors().default.dim(result.error || '')}.`);
...
function createInstallError(error) {
  ...
  if (stderr.includes('No connected devices')) {
    ...
    message = `Please accept all necessary Android SDK licenses using Android SDK Manager: "${_picocolors().default.bold('$ANDROID_HOME/tools/bin/sdkmanager --licenses')}."`;
  } else if (stderr.includes('requires Java')) {
    message = `Looks like your Android environment is not properly set. Please go to ${_picocolors().default.dim(_picocolors().default.underline(_cliTools().link.docs('set-up-your-environment', 'android', {
      hash: 'jdk-studio',
      guide: 'native'
