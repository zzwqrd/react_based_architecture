...
const printCategory = ({
  ...
  _cliTools().logger.log(_picocolors().default.dim(label));
};
const printVersions = ({
  version,
  versions,
  ...
  if (versions) {
    ...
    (0, _common.logMessage)(`- Versions found: ${_picocolors().default.red(versionsToShow)}`);
    (0, _common.logMessage)(`- Version supported: ${_picocolors().default.green(versionRange)}`);
    return;
  }
  const versionsToShow = version && version !== 'Not Found' ? version : 'N/A';
  (0, _common.logMessage)(`- Version found: ${_picocolors().default.red(versionsToShow)}`);
  (0, _common.logMessage)(`- Version supported: ${_picocolors().default.green(versionRange)}`);
  return;
};
const printIssue = ({
  ...
  const symbol = needsToBeFixed ? isRequired ? _picocolors().default.red('✖') : _picocolors().default.yellow('●') : _picocolors().default.green('✓');
...
const printOverallStats = ({
  ...
  _cliTools().logger.log(`\n${_picocolors().default.bold('Errors:')}   ${errors}`);
  _cliTools().logger.log(`${_picocolors().default.bold('Warnings:')} ${warnings}`);
};
/**
