...
const logManualInstallation = ({
  ...
  if (url) {
    logMessage(`Read more about how to download ${healthcheck} at ${_picocolors().default.dim(_picocolors().default.underline(url))}`);
    return;
  }
  if (command) {
    logMessage(`Please install ${healthcheck} by running ${_picocolors().default.bold(command)}`);
  }
};
...
const logError = ({
  ...
  addBlankLine();
  logMessage(_picocolors().default.dim(error.message));
  if (message) {
    logMessage(message);
    addBlankLine();
    return;
  }
  logMessage(`The error above occured while trying to install ${healthcheck}. Please try again manually: ${_picocolors().default.bold(command)}`);
  addBlankLine();
};
